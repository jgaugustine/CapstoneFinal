...
export const TutorialTour = ({
  ...
  if (targetRect && !useCenteredLayout) {
    ...
    const needsExtraUpOffset = ["add-adjustment-btn", "layer-list", "transform-controls"].includes(step.target);
    const extraUpOffset = needsExtraUpOffset ? 48 : 0;

    if (canPlaceAbove || (!canPlaceBelow && aboveSpace >= belowSpace) || (needsExtraUpOffset && aboveSpace >= 120)) {
      cardTop = Math.max(spacing, targetRect.top - estimatedCardHeight - spacing - extraUpOffset);
    } else {
